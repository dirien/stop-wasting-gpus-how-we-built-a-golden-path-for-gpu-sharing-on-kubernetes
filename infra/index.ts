...
const gpuSubnetIds = vpc.privateSubnetIds.apply(async (ids) => {
    const results = await Promise.all(
        ids.map(async (id) => {
            const s = await aws.ec2.getSubnet({ id });
...
// When an MNG uses a Launch Template, disk size MUST be defined here
// (the MNG's diskSize field is ignored / rejected by the EKS API).
const gpuLaunchTemplate = new aws.ec2.LaunchTemplate("gpu-cb-lt", {
    namePrefix: "gpu-cb-lt-",
    // For CAPACITY_BLOCK MNGs, instance type MUST be in the launch template
    // (and MUST NOT be in the MNG's instanceTypes field).
    instanceType: "p4d.24xlarge",
    // Required by AWS to launch instances into an EC2 Capacity Block reservation.
...
const gpuNodeGroup = new eks.ManagedNodeGroup("gpu-nodes", {
    ...
    // instance type lives in the launch template (CAPACITY_BLOCK requirement)
    capacityType: "CAPACITY_BLOCK",
    launchTemplate: {
        id: gpuLaunchTemplate.id,
        version: gpuLaunchTemplate.latestVersion.apply((v) => v.toString()),
    },
    scalingConfig: {
        desiredSize: 1,
        minSize: 1,
...
const coreDnsAddon = new aws.eks.Addon("coredns", {
    ...
    addonName: "coredns",
    addonVersion: "v1.14.2-eksbuild.4",
...
const ebsCsiAddon = new aws.eks.Addon("aws-ebs-csi-driver", {
    ...
    addonVersion: "v1.59.0-eksbuild.1",
    resolveConflictsOnCreate: "OVERWRITE",
    resolveConflictsOnUpdate: "OVERWRITE",
...
for epoch in range(100000):
    ...
    print(f"Epoch {epoch+1} | Loss: {avg_loss:.4f} | Accuracy: {accuracy:.2f}% | GPU Mem: {torch.cuda.memory_allocated()/1e9:.2f}GB")
...
// Shared ResourceClaim with three sub-requests + a matchAttributes constraint.
// The constraint binds all three slices to the SAME parent physical GPU —
// the scheduler resolves the parentUUID at allocation time, so this works
// across redeploys without hardcoding any UUID we can't know upfront.
//
// Each of the three demo pods references this single claim by name and
// picks its sub-request via `containers[].resources.claims[].request`.
const sharedMigClaim = new k8s.resource.v1.ResourceClaim("shared-mig-claim", {
    metadata: {
...
const migLargeTrainingPod = new k8s.core.v1.Pod("mig-large-training", {
    ...
    spec: {
        ...
        containers: [{
            ...
            resources: {
                claims: [{ name: "mig-large", request: "mig-large" }],
            },
        }],
        resourceClaims: [{
            name: "mig-large",
...
}, { provider: k8sProvider, dependsOn: [sharedMigClaim, fashionMnistScripts] });
...
const migMediumTrainingPod = new k8s.core.v1.Pod("mig-medium-training", {
    ...
    spec: {
        ...
        containers: [{
            ...
            resources: {
                claims: [{ name: "mig-medium", request: "mig-medium" }],
            },
        }],
        resourceClaims: [{
            name: "mig-medium",
...
const migSmallInferencePod = new k8s.core.v1.Pod("mig-small-inference", {
    ...
    spec: {
        ...
        containers: [{
            ...
            resources: {
                claims: [{ name: "mig-small", request: "mig-small" }],
            },
        }],
        resourceClaims: [{
            name: "mig-small",
...
/*
const migInvalidClaimTemplate = new k8s.resource.v1.ResourceClaimTemplate("mig-invalid-template", {
    metadata: {
